...
def construct_golomb(n_marks=8):
    ...
    # Variables
    upper_bound = n_marks * n_marks if n_marks > 8 else 25
    grid = cp.intvar(1, upper_bound, shape=(1, n_marks), name="grid")
    ...
    lang = [
        ...
        AV[0] < AV[1], AV[0] > AV[1], 
        AV[0] >= AV[1], AV[0] <= AV[1], 
        cp.abs(AV[0] - AV[1]) != cp.abs(AV[2] - AV[3]),
        #cp.abs(AV[0] - AV[1]) == cp.abs(AV[2] - AV[3])

        # The '==' version is removed because finding a valid layout where 
        # two distances ARE equal is actually harder than finding one where they differ,
        # often leading to timeouts and false positives.
    ]
