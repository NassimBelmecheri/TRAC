...
class ActiveCAEnv(CAEnv):
    ...
    def ask_membership_query(self, Y=None):
        ...
        if self.metrics:
            # 1. Get all variable objects from the problem instance (flattened)
            # This ensures consistent column order in the CSV
            all_vars = self.instance.X
            
            # 2. Extract Names (headers)
            var_names = [v.name for v in all_vars]
            
            # 3. Extract Values
            # Logic: If variable 'v' is in the current query scope 'Y', record its value.
            #        Otherwise, record 0.
            
            # Create a set for O(1) lookup speed
            query_scope_set = set(Y)
            
            var_vals = []
            for v in all_vars:
                # Check membership in Y AND if value exists
                if v in query_scope_set and v.value() is not None:
                    var_vals.append(v.value())
                else:
                    var_vals.append(0)
            
            # 4. Send to metrics
            self.metrics.record_query_data(var_names, var_vals, self.last_answer)
        if self.verbose >= 3:
