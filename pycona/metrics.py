...
class Metrics:
    ...
    def write_queries_to_file(self, filename):
        ...
        try:
            ...
            warnings.warn("pandas library not installed! Cannot write to file")
    # NEW: Method to record a row
    def record_query_data(self, var_names, var_values, label):
        """
        :param var_names: List of strings (column headers)
        :param var_values: List of values (row data)
        :param label: The oracle answer (True/False)
        """
        # Create a dictionary for this row
        row = dict(zip(var_names, var_values))
        # Add the label as the last column
        row['label'] = 1 if label else 0
        
        self.dataset_buffer.append(row)

    # NEW: Method to save to CSV using Pandas
    def save_dataset_to_csv(self, filename="generated_dataset.csv"):
        try:
            if not self.dataset_buffer:
                print("No queries recorded to save.")
                return

            # Create DataFrame
            df = pd.DataFrame(self.dataset_buffer)
            
            # Save to CSV
            df.to_csv(filename, index=False)
            print(f"Dataset saved to {filename} ({len(df)} rows)")
            
        except ImportError:
            print("Error: Pandas is not installed. Cannot save CSV.")
